...
- New `cli-walkthrough` skill at `.claude/skills/cli-walkthrough.md` — pre-publish exhaustive simulation of every CLI command across the realistic input matrix (recovery-panel 4×2 cells included). Wired into `/release-flow` as Step 4c. Caught the pool-topup regression (cells D and F) before 1.0.4 reached npm.
- New `test/pool-topup.test.js` (11 unit cases) for the top-up generator. New 8-cell matrix in `test/validator-recovery-pool-filter.test.js` asserting option 1 either contains exactly 3 `--keywords` items or is fully suppressed. `test/suggested-commands-resolvable.test.js` extended with a per-suggestion `--keywords` item-count assertion.
