...
// ─────────────────────────────────────────────────────────────────────────────
// Scores (UVI components — re-derived from results, not from summary which only
// stores composite). Mirrors lib/report/visibility-index.js logic.

function scores(summary) {
  // Composite UVI lives at summary.score (already 0–100 in current tracker).
  // Sub-component breakdown is computed from results.
  const results = summary.results || [];
  const cells = results.length;
  if (cells === 0) {
    return { uvi: 0, presence: 0, sentiment: null, rank: null, citation: null };
  }

  const yesCount = results.filter(r => r.mention === 'yes').length;
  const srcCount = results.filter(r => r.mention === 'src').length;
  const presence = Math.round(((yesCount + 0.5 * srcCount) / cells) * 100);

  const sentRated = results
    .map(r => r.sentiment?.label)
    .filter(s => s === 'positive' || s === 'neutral' || s === 'negative');
  const sentiment = sentRated.length === 0 ? null : Math.round(
    (sentRated.filter(s => s === 'positive').length * 100 +
     sentRated.filter(s => s === 'neutral').length * 50) / sentRated.length
  );

  const ranked = results.filter(r => r.position != null && r.position > 0);
  const rank = ranked.length === 0 ? null : Math.round(
    ranked.reduce((acc, r) => acc + Math.max(0, 100 - (r.position - 1) * 10), 0) / ranked.length
  );

  const cited = results.filter(r => r.hasBrandInCitations === true).length;
  const citation = Math.round((cited / cells) * 100);

  return {
    uvi: numOr(summary.score, 0),
    presence,
    sentiment,
    rank,
    citation,
...
function perEngine(summary) {
  ...
  return Array.from(byProvider.values()).map(p => {
    ...
    const yesCount = cells.filter(c => c.mention === 'yes').length;
    const srcCount = cells.filter(c => c.mention === 'src').length;
    const presence = total === 0 ? 0 : Math.round(((yesCount + 0.5 * srcCount) / total) * 100);

    const sentRated = cells
      .map(c => c.sentiment?.label)
      .filter(s => s === 'positive' || s === 'neutral' || s === 'negative');
    const sentiment = sentRated.length === 0 ? null : Math.round(
      (sentRated.filter(s => s === 'positive').length * 100 +
       sentRated.filter(s => s === 'neutral').length * 50) / sentRated.length
    ...
    const ranked = cells.filter(c => c.position != null && c.position > 0);
    const rank = ranked.length === 0 ? null : Math.round(
      ranked.reduce((acc, c) => acc + Math.max(0, 100 - (c.position - 1) * 10), 0) / ranked.length
    );

    const cited = cells.filter(c => c.hasBrandInCitations === true).length;
    const citation = total === 0 ? 0 : Math.round((cited / total) * 100);
