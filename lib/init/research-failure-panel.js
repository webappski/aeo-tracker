...
export function formatResearchFailurePanel({ attempts, brand, domain, useColor = true }) {
  ...
  lines.push(`    ${c.bold}${optNum}.${c.reset} Skip brainstorm — provide exactly 3 queries yourself (zero LLM cost):`);
  lines.push(`         ${c.dim}aeo-platform init --yes \\${c.reset}`);
  lines.push(`         ${c.dim}  --brand=${brand} \\${c.reset}`);
  lines.push(`         ${c.dim}  --domain=${domain} \\${c.reset}`);
  // 1.0.6 cli-walkthrough adversarial probe catch: the CLI's --keywords gate
  // requires exactly 3 comma-separated entries (bin/aeo-tracker.js:1185).
  // The legacy template used 5 placeholders here — copy-pasting it would
  // hit the same trust-failure class the 1.0.2-1.0.6 patches addressed.
  lines.push(`         ${c.dim}  --keywords="query 1,query 2,query 3"${c.reset}`);
  lines.push('');
  ...
  if (unsettable && attempts.length > 1) {
    ...
    // 1.0.6: --manual in --yes mode needs --keywords too, otherwise queries
    // stay empty and init errors at the no-queries gate. Append placeholder
    // matching the unexpected-error panel's pattern.
    lines.push(`         ${c.dim}  --brand=${brand} --domain=${domain} --manual --keywords="query 1,query 2,query 3"${c.reset}`);
    lines.push('');
  }
