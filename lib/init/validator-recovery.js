...
export function formatRecoveryPanel({
  ...
  const lines = [];
  lines.push('');
  lines.push(`${c.red}${c.bold}  Cannot auto-recover — ${dedupedBlockers.length} query/queries blocked by validator.${c.reset}`);
  lines.push('');
  ...
  // Option 1 is suppressed entirely when there are no validated suggestions
  // AND no clean category — the command would emit nothing useful and
  // mislead the operator. Suppression renumbers options 2/3/4 → 1/2/3.
  ...
  if (showOption1) {
    ...
    // When option 1 is suppressed, explain why so the operator isn't left
    // wondering whether the panel skipped a step.
    lines.push(`    ${c.dim}${poolNote.trim()}${c.reset}`);
    lines.push('');
  }
